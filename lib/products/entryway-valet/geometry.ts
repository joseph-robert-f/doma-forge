...
export async function generateEntrywayValet(
  ...
  try {
    ...
    const wellZoneAtOrigin = scope.own(kernel.Manifold.cube(
      [
    ...
    const wellZone = scope.own(wellZoneAtOrigin.translate([
      0,
      (layout.backDividerY - valetDepth / 2 - BOOLEAN_OVERLAP) / 2,
      valetHeight / 2,
    ]));
    scope.delete(wellZoneAtOrigin);
    const wellClip = scope.own(outer.intersect(wellZone));
    scope.delete(wellZone);
    const wellDividers = scope.own(dividerArrayAtPositions(kernel, wellClip, {
    ...
    scope.delete(wellClip);
    if (wellDividers) parts.push(wellDividers);

    const backDivider = scope.own(dividerArrayAtPositions(kernel, outer, {
    ...
    const lip = scope.own(dividerArrayAtPositions(kernel, outer, {
    ...
    const wedgeFull = scope.own(extrudeAlongX(kernel, profile, valetWidth + BOOLEAN_OVERLAP * 2));
    scope.delete(profile);
    const restOutline = scope.own(roundedRectangle(kernel, valetWidth, valetDepth, parameters.cornerRadius, segments));
    const restClip = scope.own(restOutline.extrude(parameters.restHeight));
    scope.delete(restOutline);
    const wedge = scope.own(wedgeFull.intersect(restClip));
    scope.delete(wedgeFull);
    scope.delete(restClip);
    parts.push(wedge);

    let solid = scope.own(unionSolids(kernel, scope.takeAll(parts)));
    solid = applySurfacePatterns(kernel, scope, solid, parameters.surfaceTreatments, entrywayValetSurfaceZones(parameters));
    return finishSolid(scope.take(solid), parameters, "valet");
  } finally {
    scope.dispose();
  }
}
