 ...
 * the pockets and a rim around them. The slab is deleted when a pocket is
 * cut; it is returned untouched when `planLightening` returns null.
 */
export function lightenUnderside(
  ...
  const solid = slab.subtract(pockets);
  pockets.delete();
  slab.delete();
  return { solid, plan };
}
